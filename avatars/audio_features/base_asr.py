...
class BaseASR:
    ...
    def get_audio_frame(self)->AudioFrameData:        
        try:
            if self.parent and self.parent.custom_audiotype>1: #play custom audio, prioritize finishing the custom action; the action playback can be interrupted via interrupt
    ...
    #split audio features, called by subclasses
